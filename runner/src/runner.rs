...
pub const PARACHAIN_MODULE: &str = "ClientInfo";
...
impl Runner {
    ...
    fn try_load_downloaded_binary(runner: &mut impl RunnerExt, release: &ClientRelease) -> Result<(), Error> {
        ...
        println!("release checksum {:?} ", release.checksum);
    ...
    async fn do_download_binary( bin_path: PathBuf, release: ClientRelease) -> Result<&'static str, Error>  {
        ...
        println!("file just downloaded checksum {:?} ", checksum);
        println!("release checksum {:?} ", release.checksum);
