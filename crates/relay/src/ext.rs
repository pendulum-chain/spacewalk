...
pub(crate) mod stellar_relay {
    use stellar::types::{BlockHeader, H256Le, MerkleProof, RawBlockHeader};
    ...
    pub fn initialize<T: crate::Config>(
        ...
        <stellar_relay::Pallet<T>>::initialize(relayer, block_header, block_height)
    }

    pub fn store_block_header<T: crate::Config>(relayer: &T::AccountId, block_header: BlockHeader) -> DispatchResult {
        <stellar_relay::Pallet<T>>::store_block_header(relayer, block_header)
    }
    ...
    pub(crate) fn verify_transaction_inclusion<T: crate::Config>(
        ...
        <stellar_relay::Pallet<T>>::_verify_transaction_inclusion(tx_id, merkle_proof, None)
    }

    pub fn parse_raw_block_header<T: stellar_relay::Config>(
        raw_block_header: &RawBlockHeader,
    ) -> Result<BlockHeader, DispatchError> {
        <stellar_relay::Pallet<T>>::parse_raw_block_header(raw_block_header)
    }
