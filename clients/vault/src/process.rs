...
impl SystemProcess for System {
	...
	fn process_name(&self, pid: Pid) -> Result<String, ServiceError<Error>> {
		let process = <Self as SystemExt>::process(self, pid)
			.ok_or_else(|| ServiceError::ProcessNotFound(pid.to_string()))?;
...
impl PidFile {
	pub fn new(spec_name: &String, account_id: &AccountId) -> Self {
		Self { path: Self::compute_path(spec_name, account_id), created_pidfile: false }
	}
		...
		if pid_file.path.exists() {
			...
			if sys.refresh_process(pid) {
				match pid_name_matches_existing_client(sys, pid) {
					...
					Ok(false) => (),
					// There is a very small chance that a `pid` with a running process is killed
					// exactly before `pid_name_matches_client` is run and `ProcessNotFound` is
					// thrown. This is as if the `refresh_process` check had returned `false` (the
					// pidfile process not running) so the error should not be propagated.
...
#[cfg(all(test, feature = "parachain-metadata-foucoco"))]
mod tests {
	...
	fn test_pid_file_with_same_process() {
		...
		sys.expect_process_name().times(2).returning(|_| Ok("vault".to_string()));
