...
mod tests {
	...
	use runtime::{
		AccountId, AssetMetadata, BtcAddress, BtcPublicKey, CurrencyId, ErrorCode,
		InterBtcIssueRequest, InterBtcReplaceRequest, IssueRequestStatus, RequestIssueEvent,
		StatusCode, Token, VaultId, DOT, IBTC,
	};
	...
	async fn test_get_open_process_delays_succeeds() {
		...
		let mut canceller =
			CancellationScheduler::new(parachain_rpc, 10_000, 150, AccountId::new([1u8; 32]));
	...
	async fn test_process_event_succeeds() {
		...
		parachain_rpc.expect_get_vault_issue_requests().returning(|_| {
			Ok(vec![(
				H256::from_slice(&[1; 32]),
				InterBtcIssueRequest { opentime: 10_000, ..default_issue_request() },
			)])
		});
		...
		let mut cancellation_scheduler =
			CancellationScheduler::new(parachain_rpc, 0, 0, AccountId::new([1u8; 32]));
	...
	async fn test_process_event_get_new_list() {
		...
		parachain_rpc.expect_get_vault_issue_requests().times(1).returning(|_| {
			Ok(vec![(
				H256::from_slice(&[1; 32]),
				InterBtcIssueRequest { opentime: 10, ..default_issue_request() },
			)])
		});
		...
		let mut active_processes: Vec<ActiveRequest> = vec![];
		let mut cancellation_scheduler =
			CancellationScheduler::new(parachain_rpc, 15, 0, AccountId::new([1u8; 32]));
	...
	async fn test_period_increase() {
		...
		// normally cancelling prematurely would return issue.TimeNotExpired, but that is difficult
		// to construct..
		parachain_rpc
			.expect_cancel_issue()
			.returning(|_| Err(RuntimeError::BlockNotFound));
