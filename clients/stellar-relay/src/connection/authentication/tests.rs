...
fn mock_connection_auth() -> ConnectionAuth {
    ...
    let secret =
        SecretKey::from_encoding("SCV6Q3VU4S52KVNJOFXWTHFUPHUKVYK3UV2ISGRLIUH54UGC6OPZVK2D").expect("should work");
...
fn create_valid_auth_cert() {
    ...
    let auth_cert = create_auth_cert(auth.network_id(), auth.keypair(), time_now, auth.pub_key_ecdh().clone())
...
fn expired_auth_cert() {
    ...
    let new_auth_cert = create_auth_cert(auth.network_id(), auth.keypair(), time_now, auth.pub_key_ecdh().clone())
...
fn create_valid_shared_key() {
    ...
    let bytes = base64::decode_config("SaINZpCTl6KO8xMLvDkE2vE3knQz0Ma1RmJySOFqsWk=", base64::STANDARD)
...
fn mac_test() {
    ...
    let public_network = Network::new(b"Public Global Stellar Network ; September 2015");
    let secret =
        SecretKey::from_encoding("SDAL6QYZG7O26OTLLP7JLNSB6SHY3CBZGJAWDPHYMRW2J3D2SA2RWU3L").expect("should work");
    ...
    let mac_peer_uses_to_send_us_msg =
        create_sha256_hmac(&data_message(), &peer_sending_mac_key.mac).unwrap_or(HmacSha256Mac { mac: [0; 32] });
