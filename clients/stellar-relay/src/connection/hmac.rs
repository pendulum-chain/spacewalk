...
impl HMacKeys {
    pub fn new(
        ...
        let sending = create_sending_mac_key(&shared_key, local_nonce, remote_nonce, !remote_called_us);

        let receiving = create_receiving_mac_key(&shared_key, local_nonce, remote_nonce, !remote_called_us);
...
pub fn verify_hmac(data_buffer: &[u8], mac_key_buffer: &Buffer, mac: &[u8]) -> Result<(), Error> {
    let mut hmac = HmacSha256::new_from_slice(mac_key_buffer).map_err(|_| Error::HmacInvalidLength)?;
