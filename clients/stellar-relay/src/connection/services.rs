...
pub(crate) async fn connection_handler(
	...
	let mut retry = 0;
	loop {
		match timeout(Duration::from_secs(conn.timeout_in_secs), receiver.recv()).await {
			Ok(Some(action)) => {
				retry = 0;
				_connection_handler(action, &mut conn, &mut w_stream).await?;
			},
			...
			Err(elapsed) => {
				...
				if retry >= conn.retries {
					conn.send_to_user(StellarRelayMessage::Timeout).await?;
					return Err(Error::ConnectionFailed(format!(
