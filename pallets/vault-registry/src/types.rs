...
impl<T: Config> CurrencySource<T> {
	...
	) -> Result<crate::Amount<T>, DispatchError> {
...
impl<T: Config> RichVault<T> {
	...
	pub fn issuable_tokens(&self) -> Result<Amount<T>, DispatchError> {
		...
		// secure_collateral_threshold free_collateral = collateral - used_collateral
	...
	pub(crate) fn increase_available_replace_collateral(
		&mut self,
		amount: &Amount<T>,
	) -> DispatchResult {
		self.update(|v| {
	...
	pub(crate) fn decrease_available_replace_collateral(
		&mut self,
		amount: &Amount<T>,
	) -> DispatchResult {
		self.update(|v| {
	...
	pub(crate) fn increase_active_replace_collateral(
		&mut self,
		amount: &Amount<T>,
	) -> DispatchResult {
		self.update(|v| {
	...
	pub(crate) fn decrease_active_replace_collateral(
		&mut self,
		amount: &Amount<T>,
	) -> DispatchResult {
		self.update(|v| {
	...
	pub(crate) fn set_custom_secure_threshold(
		&mut self,
		threshold: Option<UnsignedFixedPoint<T>>,
	) -> DispatchResult {
		self.update(|v| {
	...
	pub(crate) fn liquidate(&mut self) -> Result<Amount<T>, DispatchError> {
		...
		// `to_be_issued` tokens.
		let _ =
			Pallet::<T>::withdraw_replace_request(&self.data.id, &self.to_be_replaced_tokens())?;
		...
		*self = Pallet::<T>::get_rich_vault_from_id(&vault_id)?;
		...
		let collateral_for_to_be_redeemed = liquidated_collateral
			.saturating_sub(&liquidated_collateral_excluding_to_be_redeemed)?;
