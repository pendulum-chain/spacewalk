...
fn create_vault_and_issue_tokens(
	...
	let amount =
		Amount::<Test>::new(issue_tokens, <Test as currency::Config>::GetWrappedCurrencyId::get());
...
fn deposit_collateral_succeeds() {
	run_test(|| {
		...
		assert_ok!(res);
		let new_collateral =
			ext::currency::get_reserved_balance::<Test>(Token(DOT), &id.account_id);
...
fn redeem_tokens_liquidation_succeeds() {
	run_test(|| {
		...
		assert_ok!(VaultRegistry::redeem_tokens_liquidation(Token(DOT), &user_id, &wrapped(50)));
...
fn redeem_tokens_liquidation_does_not_call_recover_when_unnecessary() {
	run_test(|| {
		...
		assert_ok!(VaultRegistry::redeem_tokens_liquidation(Token(DOT), &user_id, &wrapped(10)));
		let liquidation_vault = VaultRegistry::get_rich_liquidation_vault(&DEFAULT_CURRENCY_PAIR);
...
fn test_offchain_worker_unsigned_transaction_submission() {
	...
	externalities.execute_with(|| {
		...
		assert_eq!(
			tx.call,
			crate::mock::Call::VaultRegistry(crate::Call::report_undercollateralized_vault {
				vault_id: id
			})
		);
	})
