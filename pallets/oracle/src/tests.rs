...
fn feed_values_succeeds() {
	run_test(|| {
		let key = OracleKey::ExchangeRate(CurrencyId::XCM(DOT.into()));
...
mod oracle_offline_detection {
	...
	fn basic_oracle_offline_logic() {
		run_test(|| {
			...
			set_time(0);
			feed_value(CurrencyId::XCM(DOT.into()), OracleA);
			assert_eq!(SecurityPallet::parachain_status(), StatusCode::Running);

			set_time(5);
			feed_value(CurrencyId::XCM(KSM.into()), OracleA);
			...
			feed_value_with_value(CurrencyId::XCM(DOT.into()), OracleA, 7);
			set_time(15);
			assert_eq!(SecurityPallet::parachain_status(), StatusCode::Running);
	...
	fn oracle_offline_logic_with_multiple_oracles() {
		run_test(|| {
			...
			set_time(7);
			feed_value(CurrencyId::XCM(DOT.into()), OracleB);
...
fn getting_exchange_rate_fails_with_missing_exchange_rate() {
	run_test(|| {
		...
		assert_err!(
			Oracle::wrapped_to_collateral(0, CurrencyId::XCM(DOT.into())),
			TestError::MissingExchangeRate
		);
		assert_err!(
			Oracle::collateral_to_wrapped(0, CurrencyId::XCM(DOT.into())),
			TestError::MissingExchangeRate
		);
	});
}
...
fn wrapped_to_collateral() {
	run_test(|| {
		...
		for (input, expected) in test_cases.iter() {
			let result = Oracle::wrapped_to_collateral(*input, CurrencyId::XCM(DOT.into()));
...
fn collateral_to_wrapped() {
	run_test(|| {
		...
		for (input, expected) in test_cases.iter() {
			let result = Oracle::collateral_to_wrapped(*input, CurrencyId::XCM(DOT.into()));
			assert_ok!(result, *expected);
		}
	});
}
