...
pub(crate) type DefaultVaultId<T> =
	VaultId<<T as frame_system::Config>::AccountId, currency::CurrencyId<T>>;
...
impl<T: Config> Pallet<T> {
	...
	pub fn _deposit_collateral(
		...
		let max_nominatable_collateral = ext::vault_registry::get_max_nominatable_collateral::<T>(
			&vault_backing_collateral,
			&vault_id.currencies,
		)?;
		ensure!(
	...
	fn _opt_out_of_nomination(vault_id: &DefaultVaultId<T>) -> DispatchResult {
		...
		ext::vault_registry::decrease_total_backing_collateral(
			&vault_id.currencies,
			&refunded_collateral,
		)?;
